...
execute as @a[hasitem={item=spyglass,data=777,location=slot.weapon.mainhand},tag=t:sneaking,tag=shifton,scores={doraammo=1..,dora_ct=0}] at @s run scoreboard players set @s dora_ct 60
...
execute as @e[type=boat,tag=doragun] at @s positioned ~-0.5~-999.5~-0.5 if entity @e[dx=0] positioned ~~-1.65~ if entity @e[dx=0] run scoreboard players remove @e[dx=0] bleeding 6120
execute as @e[type=boat,tag=doragun] at @s run damage @e[x=~-0.5,z=~-0.5,y=~-999.5,dx=0] 10 entity_attack entity @s
execute as @e[type=boat,tag=doragun] at @s run scoreboard players remove @e[x=~-0.5,z=~-0.5,y=~-999.5,dx=0] bleeding 4120
